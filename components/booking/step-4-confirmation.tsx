...
const ReceiptModal = ({ isOpen, onClose, data }: { isOpen: boolean, onClose: () => void, data: any }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-card rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8 space-y-6" id="receipt-content">
          <div className="text-center border-b border-dashed border-border pb-6">
            <h4 className="text-2xl font-bold text-foreground mb-1">SwiftCare</h4>
            <p className="text-sm text-muted-foreground italic">"Your Trusted Healthcare Partner"</p>
          </div>

          <div className="grid grid-cols-2 gap-y-4 text-sm">
            <div>
              <p className="text-muted-foreground uppercase text-[10px] font-bold tracking-wider mb-1">Booking Number</p>
              <p className="font-mono font-bold text-primary text-base">{data.bookingNumber}</p>
            </div>
            <div className="text-right">
              <p className="text-muted-foreground uppercase text-[10px] font-bold tracking-wider mb-1">Status</p>
            ...
            <div className="col-span-2 pt-2 border-t border-border/50">
              <p className="text-muted-foreground uppercase text-[10px] font-bold tracking-wider mb-1">Doctor</p>
              <p className="font-bold text-foreground text-lg uppercase">{data.doctor.name}</p>
              <p className="text-primary text-xs font-medium">{data.doctor.specialty} Specialist</p>
            </div>

            <div>
              <p className="text-muted-foreground uppercase text-[10px] font-bold tracking-wider mb-1">Date</p>
              <p className="font-semibold text-foreground">{data.dateTime?.fullDate || 'TBD'}</p>
            </div>
            <div className="text-right">
              <p className="text-muted-foreground uppercase text-[10px] font-bold tracking-wider mb-1">Time</p>
              <p className="font-semibold text-foreground">{data.dateTime?.time} {data.dateTime?.period}</p>
            </div>
            ...
            <div className="col-span-2 pt-4 border-t border-border/50 mt-2">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-bold text-foreground">{data.doctor.fee}</span>
              </div>
              <div className="flex justify-between items-center mb-4 text-muted-foreground/60 text-xs italic">
              ...
              <div className="flex justify-between items-center pt-4 border-t-2 border-foreground/20">
                <span className="text-lg font-black uppercase text-foreground">Total Amount</span>
                <span className="text-2xl font-black text-primary">{data.doctor.fee}</span>
              </div>
            </div>
          </div>

          <div className="bg-muted/50 p-4 rounded-xl text-[10px] text-muted-foreground leading-relaxed text-center">
            This is a computer-generated receipt. Please present your Booking Number at the clinic reception upon arrival.
          </div>
        </div>

        <div className="p-6 bg-muted/30 border-t border-border/50 flex gap-3">
          <Button variant="outline" className="flex-1 gap-2" onClick={() => window.print()}>
            <Printer className="w-4 h-4" /> Print
          </Button>
          <Button variant="ghost" className="flex-1 hover:bg-muted" onClick={onClose}>
            Close
          </Button>
        </div>
      </div>
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2">
          <Card className="p-8 mb-6 border border-border/50 shadow-xl bg-card relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-bl-full -z-0 opacity-50" />
            
            <div className="flex flex-col items-center text-center mb-8 relative z-10">
              <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center mb-4 animate-in zoom-in-50 duration-500">
                <Check className="w-10 h-10 text-primary" />
              </div>
              <h2 className="text-4xl font-black text-foreground mb-2">Success!</h2>
            ...
            <div className="space-y-6 relative z-10">
              <div className="bg-muted/40 rounded-2xl p-6 border border-border/50">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                  <div>
                    <p className="text-muted-foreground mb-1">Doctor Name</p>
                    <p className="font-bold text-foreground text-lg">{data.doctor.name}</p>
                    <p className="text-primary italic">{data.doctor.specialty} Specialist</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground mb-1">Appointment Time</p>
                    <p className="font-bold text-foreground text-lg">{data.dateTime?.time} {data.dateTime?.period}</p>
                    <p className="text-muted-foreground font-medium">{data.dateTime?.fullDate}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground mb-1">Consultation Fee</p>
                    <p className="font-bold text-foreground text-lg">{data.doctor.fee}</p>
                  ...
                  <div>
                    ...
                    <p className="font-bold text-foreground">{data.doctor.address}</p>
                  </div>
                </div>
              </div>

              <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-border/50">
                <Button onClick={() => setIsReceiptOpen(true)} className="flex-1 bg-primary hover:bg-primary/90 text-white h-12 rounded-xl font-bold gap-2">
                  <FileText className="w-5 h-5" /> View Receipt
                </Button>
                <Link href="/patient/appointments" className="flex-1">
                  <Button variant="outline" className="w-full h-12 rounded-xl font-bold border-2 hover:bg-muted">
                    Go to Appointments
                  </Button>
                </Link>
              </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 rounded-2xl border border-border/60 shadow-sm bg-card">
            <h4 className="font-bold text-foreground mb-3">Next Steps</h4>
